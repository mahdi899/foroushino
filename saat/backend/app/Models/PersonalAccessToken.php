...
/**
 * Every authenticated API request (agents, supervisors, and the polling
 * Home/LiveOps/Reports dashboards) makes Sanctum's `Guard` resolve the
 * bearer token to this model via a DB lookup by hashed token, on every
 * single request. Cache that lookup for a short window to cut one query
 * per request under load, while still invalidating immediately on logout
