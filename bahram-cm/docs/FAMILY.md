...
6. **Seed پرمیشن:** `php artisan db:seed --class=RolePermissionSeeder` (idempotent) فقط نقش‌ها و پرمیشن‌ها را همگام می‌کند — **نقش مدیران موجود را عوض نمی‌کند**. اگر نقش‌ها به‌اشتباه `super-admin` اضافه دارند: `php artisan roles:repair-extra-super-admin --dry-run` سپس بدون `--dry-run`.
