...
BRANCH="${BRANCH:-main}"
SITE_URL="${SITE_URL:-https://fashio.ir}"
CDN_URL="${CDN_URL:-https://cdn.fashio.ir}"
...
if [[ ! -f "${NGINX_SRC}" && -f /root/fashio-origin.conf ]]; then
...
echo "Site: ${SITE_URL} (via Arvan CDN → origin ${SITE_URL%://*})"
