...
class EntryLinksController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $items = FamilyEntryLink::query()
            ->with('entryEvent:id,name,external_reference,type')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return ApiResponse::success($this->present($link->fresh(['entryEvent']), 30), 201);
    }

    public function show(FamilyEntryLink $entryLink): JsonResponse
    {
        $entryLink->load('entryEvent:id,name,external_reference,type');
