...
class FamilyAssignmentService
{
    ...
    public function assign(User $user, ?EntryContext $context = null): FamilyMembership
    {
        ...
        $requestedFamilyId = $context->familyId;
        $lockSeconds = (int) config('family.assignment_lock_seconds', 10);

        $globalLock = Cache::lock('family:assignment:global', $lockSeconds);

        return $globalLock->block($lockSeconds, function () use ($user, $context, $lockSeconds, $requestedFamilyId) {
            $existing = FamilyMembership::query()
                ->where('user_id', $user->id)
                ->first();
            ...
            if ($requestedFamilyId !== null) {
                $forced = $this->tryForcedFamily($user, $context, $lockSeconds);
                if ($forced instanceof FamilyMembership) {
                    return $forced;
                }

                throw ValidationException::withMessages([
                    'family' => ['خانواده مربوط به این لینک ورود در حال حاضر عضو جدید نمی‌پذیرد.'],
                ]);
            }
    ...
    /**
     * Join a user directly into the family specified by an entry link URL.
     */
    private function tryForcedFamily(User $user, EntryContext $context, int $lockSeconds): ?FamilyMembership
    {
        $family = Family::query()->find($context->familyId);
        if (! $family) {
            return null;
        }
