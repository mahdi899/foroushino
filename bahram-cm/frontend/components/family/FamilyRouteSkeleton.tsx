...
export function FamilyRouteSkeleton() {
  return (
    <div className="flex h-full min-h-0 flex-1 animate-pulse">
      <aside
        aria-hidden
        className="hidden h-full w-[min(100%,280px)] shrink-0 flex-col border-e border-white/[0.06] bg-gradient-to-b from-[#0e131a] to-[#090c10] px-5 py-9 lg:flex"
      >
        <div className="mx-auto h-28 w-28 rounded-full bg-white/[0.06]" />
        <div className="mx-auto mt-5 h-4 w-36 rounded-full bg-white/[0.06]" />
        <div className="mx-auto mt-3 h-3 w-44 rounded-full bg-white/[0.04]" />
      </aside>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        ...
        <div className="min-h-0 flex-1 space-y-3 px-3 py-4 sm:px-4 lg:px-5 lg:py-5">
          {[0, 1, 2].map((i) => (
            <div key={i} className="h-40 rounded-2xl bg-white/[0.04] lg:h-44" />
          ))}
        </div>
      </div>
    </div>
