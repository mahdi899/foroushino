...
function useHeroEntranceVariants(isLightEntrance: boolean) {
  ...
  return { content, group, item, statsGroup, statItem };
}

/** Client-only hero copy, CTAs, and panel motion — LCP images are server-rendered separately. */
export function HeroCinematicClient() {
  ...
  const grid = (
    <motion.div
      className="grid min-w-0 grid-cols-1 gap-3 sm:gap-5 lg:grid-cols-12 lg:items-start lg:gap-x-8 lg:gap-y-10"
      initial="hidden"
      animate="show"
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          className="hero-stats-glass mx-auto mt-3 grid w-full max-w-full grid-cols-3 gap-1.5 rounded-card-lg px-2.5 py-3 sm:mt-5 sm:gap-2 sm:px-4 sm:py-5 lg:mt-8 lg:w-auto lg:max-w-none lg:px-6"
        >
