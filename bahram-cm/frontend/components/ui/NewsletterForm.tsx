...
export function NewsletterForm({
  ...
  const pillShell =
    'rounded-pill border border-bone/12 bg-charcoal/55 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]';
  ...
  return (
    <form onSubmit={onSubmit} className={cn('relative w-full min-w-0', className)}>
      ...
      <div className="flex flex-col gap-2">
        ...
        {/* ردیف دوم: کپچا */}
        {captchaRequired ? (
          <div className={cn('flex min-w-0 items-center px-3 py-2', pillShell)}>{captchaField}</div>
        ) : null}
      </div>
