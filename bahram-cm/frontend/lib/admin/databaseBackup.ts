...
export async function exportPrivateMediaBackupAction(): Promise<
  { ok: true; blob: Uint8Array; filename: string } | { ok: false; error: string }
> {
  try {
    const token = await getToken();
    const res = await fetch(`${SERVER_API_URL}/panel/settings/database-backup/export/private-media`, {
      headers: {
        Accept: 'application/zip',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
      },
    ...
    if (!res.ok) {
      ...
      const message =
        payload && typeof payload === 'object' && 'message' in payload && typeof payload.message === 'string'
          ? payload.message
          : 'خروجی گرفتن از مدارک خصوصی ناموفق بود.';
      return { ok: false, error: message };
    }

    const disposition = res.headers.get('content-disposition') ?? '';
    const match = disposition.match(/filename="?([^";]+)"?/i);
    const filename = match?.[1] ?? `private-media-backup-${new Date().toISOString().slice(0, 10)}.zip`;
    const buffer = await res.arrayBuffer();

    return { ok: true, blob: new Uint8Array(buffer), filename };
  } catch {
    return { ok: false, error: 'خروجی گرفتن از مدارک خصوصی ناموفق بود.' };
  }
}
