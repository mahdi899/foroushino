...
import { getSatApplications, getSatIntegrationConfig } from '@/lib/admin/academyData';
import { formatDate } from '@/lib/admin/academyTypes';
import { SatStatusSelect } from './SatStatusSelect';
import { SatIntegrationSettings } from './SatIntegrationSettings';
import { SatSyncStatus } from './SatSyncStatus';

export const dynamic = 'force-dynamic';

export default async function SatApplicationsPage() {
  const [{ items: applications, error }, { config: integration }] = await Promise.all([
    getSatApplications(),
    getSatIntegrationConfig(),
  ]);

  return (
    <AdminPage title="درخواست‌های سات" desc="بررسی و پاسخ به درخواست‌های ورود به دوره SAT">
      <SatIntegrationSettings initial={integration} />
      ...
      {applications.length > 0 ? (
        <Table head={['نام', 'موبایل', 'شهر', 'سن', 'وضعیت', 'ارسال به سات', 'تاریخ ثبت']}>
          {applications.map((a) => (
            <tr key={a.id} className="hover:bg-surface-soft/40">
              ...
              <td className="px-4 py-3"><SatSyncStatus application={a} /></td>
