#!/usr/bin/env node
/**
 * Serve production build with dual-domain local proxies:
 *   app.lvh.me:3000  → Next (:3010)
 *   club.lvh.me:3001 → Next (:3010)
 *
 * Prerequisite: npm run build
 * Run: node scripts/start-local.mjs
 */
...
function proxyHttp(req, res, virtualHost, publicPort) {
  ...
  upstream.on('error', () => {
    if (!res.headersSent) {
      ...
      res.end(`Next.js روی :${NEXT_PORT} آماده نیست.`);
    }
  });
  req.pipe(upstream);
}
...
function createHostProxy(publicPort, virtualHost, label) {
  ...
  server.listen(publicPort, '0.0.0.0', () => {
    console.log(`  ${label}: http://${virtualHost}:${publicPort}  (و http://127.0.0.1:${publicPort})`);
  });
  return server;
}
...
const nextProc = spawn(
  process.execPath,
  [nextBin, 'start', '-p', String(NEXT_PORT), '-H', '127.0.0.1'],
  {
    cwd: frontendRoot,
    stdio: 'inherit',
    env: { ...process.env },
