...
- **ورود:** همان جریان ورود پنل ادمین (ایمیل/رمز → کد OTP پیامکی در صورت نیاز → توکن Sanctum). اطلاعات ادمین پیش‌فرض در `lib/config/bootstrap_admin.dart` (فقط prefill در debug).
...
- **نظرات:** هاب بر اساس خانواده/پست با «مشاهده کامنت‌ها»، تب‌های در انتظار/تأییدشده/رد‌شده/مهم/خوانده‌نشده، تأیید/رد (با دلیل)، تأیید دسته‌جمعی، مهم‌کردن (بوردر طلایی در فید خانواده)، پاسخ متنی بهرام.
...
lib/
  ...
  config/bootstrap_admin.dart      اطلاعات ادمین پیش‌فرض (انگلیسی — فقط prefill در debug)
...
```bash
flutter build apk --dart-define=API_BASE_URL=https://rostami.app/api/v1
```
