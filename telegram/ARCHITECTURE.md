...
- **به‌روزرسانی دیتا:** push از ایران به `host-sync.php` (پس‌زمینه روی هاست).
...
| **Push ایران** | `refresh_catalog` / `push_account` و… |
| **صف ایران** | relay غیرهم‌زمان — UI متوقف نمی‌شود |
...
| **Pull** | فقط **`cron/pull-sync.php`** (هر ۲ دقیقه) — نه در webhook |
