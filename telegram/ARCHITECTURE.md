...
- **ثبت‌نام / پرداخت / پشتیبانی زنده:** در صف `pending_iran_updates` → `BackgroundIranRelay` وقتی ایران در دسترس بود (بعد از پاسخ به کاربر).

| لایه | نقش |
|------|-----|
| **MySQL هاست** | کاتالوگ، پیام‌ها، حساب کاربر (snapshot)، منو |
...
منو، دوره/سمینار، حساب (از کش)، خانواده/معرفی (از کش)، عضویت کانال (کش API تلگرام).

### فقط وقتی ایران وصل است (پس‌زمینه یا با پیام آفلاین)
ثبت‌نام کامل OTP، پرداخت، پشتیبانی متصل به تیکت، سات، ادمین.
