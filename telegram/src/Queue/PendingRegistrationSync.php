...
/**
 * Queues host-local registration steps for background reconcile on Iran.
 */
final class PendingRegistrationSync
{
    ...
    public function enqueue(int $telegramUserId, array $payload): void
    {
        if ($telegramUserId <= 0) {
            return;
        }
        ...
        try {
            ...
            $stmt = $this->pdo->prepare(
                'INSERT INTO pending_registration_sync (telegram_user_id, payload_json, attempts, created_at, updated_at)
            ...
            error_log('[telegram-host] registration queue push: '.$e->getMessage());
        }
    }

    /** @return list<array{id: int, telegram_user_id: int, payload: array<string, mixed>}> */
    ...
    public function markFailed(int $id, string $error): void
    {
        try {
            $stmt = $this->pdo->prepare(
                'UPDATE pending_registration_sync SET attempts = attempts + 1, last_error = :err, updated_at = NOW() WHERE id = :id',
            );
    ...
    {
        try {
            $stmt = $this->pdo->prepare('DELETE FROM pending_registration_sync WHERE id = :id');
            $stmt->execute(['id' => $id]);
        } catch (\Throwable) {
        }
    }
