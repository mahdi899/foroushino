...
final class AccountCache
{
    ...
    public function store(int $telegramUserId, array $account): void
    {
        ...
        $stmt = $this->pdo->prepare(
            'INSERT INTO telegram_accounts_cache (
             ...
             ON DUPLICATE KEY UPDATE
                ...
                verification_level = IF(:verification_level2_check IS NOT NULL, :verification_level2_val, verification_level),
                ...
                profile_json = IF(:profile2_check IS NOT NULL, :profile2_val, profile_json),
                referral_json = IF(:referral2_check IS NOT NULL, :referral2_val, referral_json),
                family_json = IF(:family2_check IS NOT NULL, :family2_val, family_json),
                owned_presents_json = IF(:presents2_check IS NOT NULL, :presents2_val, owned_presents_json),
                sat_json = IF(:sat2_check IS NOT NULL, :sat2_val, sat_json),
        ...
        $stmt->execute(array_merge($params, [
            ...
            // Native MySQL prepares forbid reusing the same named placeholder twice.
            'verification_level2_check' => $verificationLevel,
            'verification_level2_val' => $verificationLevel,
            'is_admin2' => $params['is_admin'],
            'snap_rev2' => $snapRev,
            'owned2' => $ownedJson,
            'profile2_check' => $profileJson,
            'profile2_val' => $profileJson,
            'referral2_check' => $referralJson,
            'referral2_val' => $referralJson,
            'family2_check' => $familyJson,
            'family2_val' => $familyJson,
            'presents2_check' => $presentsJson,
            'presents2_val' => $presentsJson,
            'sat2_check' => $satJson,
            'sat2_val' => $satJson,
            'snap_at2' => $snapAt,
            'hot_at2' => $hotAt,
